...
from models import db, User, Vendor, Tender, Bid
...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///tender_management.db'
...
def register():
    if request.method == 'POST':
        ...
        password = request.form.get('password')  # Store the plain-text password
...
def login():
    if request.method == 'POST':
        ...
        if user and user.password == password:  # No password hashing, direct comparison
